...
The `gpu-serving` profile name is intentionally retained as a recognized target class without a reference target context fixture. It reserves stable vocabulary for a common deployment target while avoiding untested compatibility claims. Processors MAY accept ADS documents that name this profile, but they MUST still validate against a concrete target context before planning.
...
`serverless-auxiliary` is intended for event handlers, hooks, scheduled jobs, and burst workers. It SHOULD be able to satisfy short-lived container runtime execution, event-driven scaling, secret injection, platform traces, metrics, audit export, bounded retry behavior, and restricted outbound egress when the platform is configured with the required integrations. It MUST report an incompatibility when a document requires behavior the target cannot preserve, such as durable in-process sessions, local tool servers, approval handlers, policy decision points, or signed-artifact enforcement.

`air-gapped` is intended for deployments without direct external network access. It SHOULD be able to satisfy internal artifact retrieval, internal secret stores, local model or tool dependencies, local observability sinks, offline approval and policy handlers, restricted egress, and offline supply-chain evidence when the environment is configured with those controls. It MUST report an incompatibility when a document depends on public model APIs, public container registries, hosted approval systems, external telemetry endpoints, or supply-chain verification unavailable inside the target environment.
...
|---|---|---|---|
| `air-gapped` | `container-runtime`, `secret-injection`, `persistent-storage`, `network-policy`, `outbound-egress-policy`, `trace-export`, `metrics-export`, `audit-log-export`, `human-approval`, `policy-decision`, `image-signature-verification`, and namespaced capabilities for internal artifact and model serving | Cannot depend on public model APIs, public container registries, hosted approval systems, external telemetry endpoints, or online verification services unless mirrored internally. | `examples/air-gapped.yaml` is expected to pass against `contexts/air-gapped.yaml`; examples that require `api.openai.com` or unsupported online integrations are expected to fail with compatibility diagnostics. |
| `compose-single-host` | `container-runtime`, `secret-injection`, `persistent-storage`, basic `trace-export`, `metrics-export`, `audit-log-export`, local `human-approval`, local `policy-decision` | Should not be assumed to provide native `network-policy`, `horizontal-scaling`, `event-driven-scaling`, admission control, signed artifact enforcement, cluster RBAC, or production-grade default-deny egress without external controls. | `examples/minimal.yaml` and `examples/approval-policy.yaml` are expected to pass against `contexts/compose-single-host.yaml`; `examples/serverless-auxiliary.yaml`, `examples/stateful-agent.yaml`, `examples/multi-agent.yaml`, and `examples/supply-chain.yaml` are expected to fail with compatibility diagnostics. |
