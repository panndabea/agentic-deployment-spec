...
The `air-gapped` and `gpu-serving` profile names are intentionally retained as recognized target classes without reference target context fixtures. They reserve stable vocabulary for common deployment targets while avoiding untested compatibility claims. Processors MAY accept ADS documents that name these profiles, but they MUST still validate against a concrete target context before planning.
...
| `air-gapped` | Local artifact sources, internal secret stores, internal observability sinks, restricted egress controls, policy enforcement, and audit export to local systems | Cannot depend on public model APIs, public container registries, hosted approval systems, or external telemetry endpoints unless mirrored internally. | Recognized profile name without a v1.0 target context fixture. |
