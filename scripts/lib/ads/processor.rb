...
SANDBOX_LEVELS = %w[restricted baseline privileged].freeze
POLICY_DEFAULTS = %w[allow deny].freeze
SECRET_ROTATION_VALUES = %w[required recommended manual not-supported].freeze
SECRET_RELOAD_VALUES = %w[hot-reload restart-required not-applicable].freeze
RESOURCE_NAME_MAX_LENGTH = 63

def redact_secret_values(value)
  Ads::Diagnostic.redact(value)
end

def redacted_secret_binding(binding)
  Ads::Diagnostic.redact(binding || {}, binding_allowlist: true)
end

def invocation_diagnostic(path, message)
  {
    "category" => "invocation-invalid",
    "severity" => "error",
    "path" => path,
    "message" => message
  }
...
def diagnostic_errors?(diagnostics)
...
def schema_error(diagnostics, path, message)
  add_diagnostic(
    diagnostics,
    category: "schema-invalid",
    severity: "error",
...
def validate_string_list(value, diagnostics, path)
  unless value.is_a?(Array)
    schema_error(diagnostics, path, "#{path} must be a list.")
    return
  end

  value.each_with_index do |entry, index|
    next if non_empty_string?(entry)

    schema_error(diagnostics, "#{path}[#{index}]", "#{path}[#{index}] must be a non-empty string.")
  end
end

def validate_capability_entries(value, diagnostics, path)
  unless value.is_a?(Array)
    schema_error(diagnostics, path, "#{path} must be a list.")
    return
  end
  ...
  value.each_with_index do |entry, index|
    case entry
    when String
      schema_error(diagnostics, "#{path}[#{index}]", "Capability name must be non-empty.") if entry.empty?
    when Hash
      schema_error(diagnostics, "#{path}[#{index}].name", "Capability entry must include name.") unless non_empty_string?(entry["name"])
      if entry.key?("for")
        scoped = entry["for"]
        if scoped.is_a?(Array)
          validate_string_list(scoped, diagnostics, "#{path}[#{index}].for")
        elsif !non_empty_string?(scoped)
          schema_error(diagnostics, "#{path}[#{index}].for", "Capability for must be a string or list of strings.")
        end
      end
    else
      schema_error(diagnostics, "#{path}[#{index}]", "Capability entry must be a string or mapping.")
    end
  end
end

def check_structural_schema(document, diagnostics)
  unless document.is_a?(Hash)
    schema_error(diagnostics, "$", "ADS document must be a mapping.")
    return
  end

  REQUIRED_ROOT_FIELDS.each do |field|
    schema_error(diagnostics, "$.#{field}", "Missing required root field #{field}.") unless document.key?(field)
  end

  api_version = document["apiVersion"]
  schema_error(diagnostics, "$.apiVersion", "apiVersion must be ads.dev/v1.") if !api_version.nil? && api_version != "ads.dev/v1"

  kind = document["kind"]
  schema_error(diagnostics, "$.kind", "kind must be AgenticDeployment.") if !kind.nil? && kind != "AgenticDeployment"

  metadata = document["metadata"]
  if metadata.is_a?(Hash)
    schema_error(diagnostics, "$.metadata.name", "metadata.name is required.") unless non_empty_string?(metadata["name"])
    schema_error(diagnostics, "$.metadata.owner", "metadata.owner is required.") unless non_empty_string?(metadata["owner"])
  elsif !metadata.nil?
    schema_error(diagnostics, "$.metadata", "metadata must be a mapping.")
  end

  profiles = document["profiles"]
  if profiles
    validate_string_list(profiles, diagnostics, "$.profiles")
    if profiles.is_a?(Array) && profiles.uniq.length != profiles.length
      schema_error(diagnostics, "$.profiles", "profiles entries must be unique.")
    end
  end

  runtime = document["runtime"]
  components = runtime.is_a?(Hash) ? runtime["components"] : nil
  if runtime && !runtime.is_a?(Hash)
    schema_error(diagnostics, "$.runtime", "runtime must be a mapping.")
  elsif !components.is_a?(Array) || components.empty?
    schema_error(diagnostics, "$.runtime.components", "runtime.components must be a non-empty list.")
  end

  if components.is_a?(Array)
    components.each_with_index do |component, index|
      base = "$.runtime.components[#{index}]"
      unless component.is_a?(Hash)
        schema_error(diagnostics, base, "Component must be a mapping.")
        next
      end

      schema_error(diagnostics, "#{base}.name", "Component name is required.") unless non_empty_string?(component["name"])
      if non_empty_string?(component["type"]) && !COMPONENT_TYPES.include?(component["type"])
        schema_error(diagnostics, "#{base}.type", "Component type #{component["type"].inspect} is not recognized.")
      elsif !non_empty_string?(component["type"])
        schema_error(diagnostics, "#{base}.type", "Component type is required.")
      end

      unless non_empty_string?(component["image"]) || non_empty_string?(component["externalRef"])
        schema_error(diagnostics, base, "Component must declare image or externalRef.")
      end

      execution = component["execution"]
      if execution.is_a?(Hash)
        mode = execution["mode"]
        if non_empty_string?(mode) && !EXECUTION_MODES.include?(mode)
          schema_error(diagnostics, "#{base}.execution.mode", "Component execution.mode #{mode.inspect} is not recognized.")
        elsif !non_empty_string?(mode)
          schema_error(diagnostics, "#{base}.execution.mode", "Component execution.mode is required.")
        end
      else
        schema_error(diagnostics, "#{base}.execution", "Component execution must be a mapping.")
      end
      ...
      unless ports.is_a?(Array)
        schema_error(diagnostics, "#{base}.ports", "Component ports must be a list.")
        next
      end

      ports.each_with_index do |port, port_index|
        port_path = "#{base}.ports[#{port_index}]"
        unless port.is_a?(Hash)
          schema_error(diagnostics, port_path, "Port must be a mapping.")
          next
        end

        schema_error(diagnostics, "#{port_path}.name", "Port name is required.") unless non_empty_string?(port["name"])
        container_port = port["containerPort"]
        unless container_port.is_a?(Integer) && container_port.between?(1, 65_535)
          schema_error(diagnostics, "#{port_path}.containerPort", "containerPort must be between 1 and 65535.")
        end
      end
    end
  end
  ...
  if capabilities.is_a?(Hash)
    validate_capability_entries(capabilities["required"], diagnostics, "$.capabilities.required")
    validate_capability_entries(capabilities["optional"], diagnostics, "$.capabilities.optional") if capabilities.key?("optional")
  elsif !capabilities.nil?
    schema_error(diagnostics, "$.capabilities", "capabilities must be a mapping.")
  end
  ...
  if secrets.is_a?(Hash)
    ...
    unless required.is_a?(Array)
      schema_error(diagnostics, "$.secrets.required", "secrets.required must be a list.")
    else
      required.each_with_index do |secret, index|
        path = "$.secrets.required[#{index}]"
        unless secret.is_a?(Hash)
          schema_error(diagnostics, path, "Secret entry must be a mapping.")
          next
        end
        schema_error(diagnostics, "#{path}.name", "Secret name is required.") unless non_empty_string?(secret["name"])
        schema_error(diagnostics, "#{path}.purpose", "Secret purpose is required.") unless non_empty_string?(secret["purpose"])
        rotation = secret["rotation"]
        if non_empty_string?(rotation) && !SECRET_ROTATION_VALUES.include?(rotation)
          schema_error(diagnostics, "#{path}.rotation", "Secret rotation #{rotation.inspect} is not recognized.")
        elsif !non_empty_string?(rotation)
          schema_error(diagnostics, "#{path}.rotation", "Secret rotation is required.")
        end
        reload = secret["reload"]
        if non_empty_string?(reload) && !SECRET_RELOAD_VALUES.include?(reload)
          schema_error(diagnostics, "#{path}.reload", "Secret reload #{reload.inspect} is not recognized.")
        end
      end
    end
  elsif !secrets.nil?
    schema_error(diagnostics, "$.secrets", "secrets must be a mapping.")
  end

  security = document["security"]
  if security.is_a?(Hash)
    sandbox = security["defaultSandbox"]
    if non_empty_string?(sandbox) && !SANDBOX_LEVELS.include?(sandbox)
      schema_error(diagnostics, "$.security.defaultSandbox", "security.defaultSandbox #{sandbox.inspect} is not recognized.")
    elsif !non_empty_string?(sandbox)
      schema_error(diagnostics, "$.security.defaultSandbox", "security.defaultSandbox is required.")
    end

    tool_policy = security["toolPolicy"]
    if tool_policy.is_a?(Hash)
      default = tool_policy["default"]
      if non_empty_string?(default) && !POLICY_DEFAULTS.include?(default)
        schema_error(diagnostics, "$.security.toolPolicy.default", "security.toolPolicy.default #{default.inspect} is not recognized.")
      elsif !non_empty_string?(default)
        schema_error(diagnostics, "$.security.toolPolicy.default", "security.toolPolicy.default is required.")
      end
    else
      schema_error(diagnostics, "$.security.toolPolicy", "security.toolPolicy must be a mapping.")
    end
  elsif !security.nil?
    schema_error(diagnostics, "$.security", "security must be a mapping.")
  end

  approvals = document["approvals"]
  if approvals.is_a?(Hash)
    schema_error(diagnostics, "$.approvals.required", "approvals.required must be a list.") unless approvals["required"].is_a?(Array)
  elsif !approvals.nil?
    schema_error(diagnostics, "$.approvals", "approvals must be a mapping.")
  end

  observability = document["observability"]
  if observability.is_a?(Hash)
    metrics = dig_hash(document, "observability", "metrics", "required")
    audit_events = dig_hash(document, "observability", "auditEvents", "required")
    schema_error(diagnostics, "$.observability.metrics.required", "observability.metrics.required must be a list.") unless metrics.is_a?(Array)
    if audit_events.is_a?(Array)
      audit_events.each_with_index do |event, index|
        next if audit_event_name_valid?(event)

        schema_error(diagnostics, "$.observability.auditEvents.required[#{index}]", "Audit event must be a standard ADS audit event name or a namespaced extension name.")
      end
    else
      schema_error(diagnostics, "$.observability.auditEvents.required", "observability.auditEvents.required must be a list.")
    end
  elsif !observability.nil?
    schema_error(diagnostics, "$.observability", "observability must be a mapping.")
  end

  outbound_default = dig_hash(document, "security", "outbound", "default")
  egress_default = dig_hash(document, "networking", "egress", "default")
  if outbound_default && egress_default && outbound_default != egress_default
    schema_error(diagnostics, "$.networking.egress.default", "networking.egress.default must match security.outbound.default.")
  end
end

def normalize_resource_name(name, max_length: RESOURCE_NAME_MAX_LENGTH)
  normalized = name.to_s.downcase.gsub(/[^a-z0-9-]+/, "-").gsub(/-+/, "-")
  normalized = normalized.sub(/\A-+/, "").sub(/-+\z/, "")
  normalized = "ads-resource" if normalized.empty?
  normalized = normalized[0, max_length]
  normalized = normalized.sub(/-+\z/, "")
  normalized.empty? ? "ads-resource" : normalized
end

def check_resource_name_collisions(document, diagnostics)
  components = dig_hash(document, "runtime", "components")
  return unless components.is_a?(Array)
  ...
  components.each_with_index do |component, index|
    ...
    resource_name = normalize_resource_name(name)
    previous = seen[resource_name]
    if previous && previous != name
      add_diagnostic(
        diagnostics,
        category: "processor-limitation",
        severity: "error",
        path: "$.runtime.components[#{index}].name",
        message: "Component name #{name.inspect} normalizes to generated resource name #{resource_name.inspect}, which collides with #{previous.inspect}.",
        extra: { "component" => name, "resourceName" => resource_name }
      )
    else
      seen[resource_name] = name
    end
  end
end

def check_dependency_cycles(document, diagnostics)
  components = dig_hash(document, "runtime", "components")
  return unless components.is_a?(Array)
  ...
  components.each_with_index do |component, index|
    ...
    indexes[name] = index
    graph[name] = string_or_list(component["dependsOn"]).select { |dependency| dependency.is_a?(String) }
  end
  ...
  visit = lambda do |name, stack|
    ...
    if visiting[name]
      ...
      unless reported[key]
        reported[key] = true
        index = indexes.fetch(name, 0)
        add_diagnostic(
          diagnostics,
          category: "reference-invalid",
          severity: "error",
          path: "$.runtime.components[#{index}].dependsOn",
    ...
    graph.fetch(name, []).each do |dependency|
      visit.call(dependency, stack + [name]) if graph.key?(dependency)
    end
    visiting.delete(name)
    visited[name] = true
  end

  graph.keys.each { |name| visit.call(name, []) }
end
...
def check_document(document, context)
  ...
  check_unknown_root_fields(document, diagnostics)
  check_component_references(document, diagnostics)
  check_dependency_cycles(document, diagnostics)
  check_resource_name_collisions(document, diagnostics)
  check_scoped_component_references(document, diagnostics)
  check_policy_decision_points(document, diagnostics)
  check_capability_recommendations(document, diagnostics)
  check_audit_event_names(document, diagnostics)
...
def profile_selection_diagnostics(document, context, diagnostics)
  profile = target_profile(context)
  unless non_empty_string?(profile)
    add_diagnostic(
      diagnostics,
      category: "processor-limitation",
      severity: "error",
      path: "$.targetProfile",
      message: "Target context must declare a concrete targetProfile."
    )
    return
  end
  ...
  add_diagnostic(
    ...
    severity: "error",
    path: "$.profiles",
    message: "Selected targetProfile #{profile.inspect} is not listed in ADS profiles.",
    extra: { "targetProfile" => profile }
  )
end

def validation_result(file:, context_file:, document:, context:, diagnostics:, strict_warnings:)
  Ads::ValidationResult.new(
    file: file,
    context_file: context_file,
    document: document,
    context: context,
    target_profile: target_profile(context),
    diagnostics: redact_secret_values(diagnostics),
    strict_warnings: strict_warnings
  )
end

def validate_document(file:, context_file: nil, strict_warnings: false, planning: false)
  document = nil
  context = nil
  diagnostics = []

  begin
    document = load_yaml(file)
  rescue StandardError => e
    diagnostics << invocation_diagnostic("$", e.message)
  ...
  check_structural_schema(document, diagnostics)
  return validation_result(file: file, context_file: context_file, document: document, context: nil, diagnostics: diagnostics, strict_warnings: strict_warnings) if diagnostic_errors?(diagnostics)

  diagnostics.concat(check_document(document, nil))
  return validation_result(file: file, context_file: context_file, document: document, context: nil, diagnostics: diagnostics, strict_warnings: strict_warnings) if diagnostic_errors?(diagnostics)

  if context_file
    begin
      context = load_target_context(context_file)
    rescue StandardError => e
      diagnostics << invocation_diagnostic("$.context", e.message)
      return validation_result(file: file, context_file: context_file, document: document, context: nil, diagnostics: diagnostics, strict_warnings: strict_warnings)
    end

    profile_selection_diagnostics(document, context, diagnostics) if planning
    check_target_context(document, context, diagnostics)
  end
...
def validate_for_planning(file:, context_file:, strict_warnings: false)
  unless context_file
    diagnostics = [invocation_diagnostic("$.context", "Planning requires exactly one target context.")]
    return validation_result(
      file: file,
      context_file: context_file,
      document: nil,
...
def secret_binding_for(context, name)
  bindings = dig_hash(context, "secrets", "bindings")
  case bindings
  when Hash
    binding = bindings[name]
    return [binding, "$.secrets.bindings.#{name}"] if binding.is_a?(Hash)
  when Array
    bindings.each_with_index do |binding, index|
      next unless binding.is_a?(Hash) && binding["name"] == name

      return [binding, "$.secrets.bindings[#{index}]"]
    end
  end

  external = dig_hash(context, "externalIntegrations", "secrets")
  case external
  when Hash
    binding = external[name]
    return [binding, "$.externalIntegrations.secrets.#{name}"] if binding.is_a?(Hash)
  when Array
    external.each_with_index do |binding, index|
      next unless binding.is_a?(Hash) && binding["name"] == name

      return [binding, "$.externalIntegrations.secrets[#{index}]"]
    end
  end
